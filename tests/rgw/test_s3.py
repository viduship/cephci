...
def create_s3_user(node: CephNode, user_prefix: str, data: Dict) -> None:
    ...
    if display_name == "iamrootalt-user":
        cmd = f"radosgw-admin  user create--uid=iamrootalt-user --account-id RGW22222222222222222 --account-root --display_name={display_name}"
        cmd += " --email={email}@foo.bar".format(email=uid)
    elif display_name == "iamroot-user":
        cmd = f"radosgw-admin  user create--uid=iamroot-user  --account-id RGW11111111111111111  --account-root --display_name={display_name}"
        cmd += " --email={email}@foo.bar".format(email=uid)
    else:
